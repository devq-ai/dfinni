#!/usr/bin/env python3
"""Load all available data into SurrealDB tables"""
...
async def load_data():
    ...
    async with httpx.AsyncClient() as client:
        print("=" * 80)
        print("LOADING ALL DATA INTO SURREALDB")
        print("=" * 80)
        ...
        # 2. Load admin user
        print("\n2. Loading admin user...")
        admin_query = f"""
        USE NS {NAMESPACE} DB {DATABASE};
        INSERT INTO user {{
            id: 'admin',
            email: 'admin@example.com',
            password_hash: '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewKyNiGSou3dFKK',
            first_name: 'Admin',
            last_name: 'User',
            role: 'ADMIN',
            is_active: true,
        ...
        response = await client.post(SURREAL_URL, headers=headers, data=admin_query)
        ...
        response = await client.post(SURREAL_URL, headers=headers, data=audit_query)
        if response.status_code == 200:
        ...
        INSERT INTO alert {{
            ...
            message: 'Sarah Anderson has a birthday next week',
            patient_id: 'patient:q7coj9uw14hiojr9ujag',
            is_read: false,
            created_at: time::now()
        }};
        INSERT INTO alert {{
            ...
            message: 'Michael Johnson status changed from Active to Churned',
            patient_id: 'patient:v82z1wpxgrg5gfncpvmb',
            is_read: false,
            created_at: time::now()
        }};
        INSERT INTO alert {{
            ...
            message: 'James Brown requires immediate attention',
            patient_id: 'patient:dpe9x0o245yj8hdxgdck',
            is_read: false,
            created_at: time::now()
        }};
        """
                ...
                if len(result) > 1 and result[1].get('result'):
                    ...
                    if sample_response.status_code == 200:
                        ...
                        if len(sample_result) > 1 and sample_result[1].get('result'):
                            ...
                            for i, record in enumerate(sample_result[1]['result'][:3]):
                                if table == 'patient':
                                    print(f"  {i+1}. {record.get('first_name')} {record.get('last_name')} - {record.get('status')} ({record.get('risk_level')} risk)")
                                elif table == 'user':
                                    print(f"  {i+1}. {record.get('email')} - {record.get('role')} - Active: {record.get('is_active')}")
                                elif table == 'audit_log':
                                    print(f"  {i+1}. {record.get('action')} on {record.get('resource_type')} by {record.get('user_email')}")
                                elif table == 'alert':
                                    print(f"  {i+1}. [{record.get('priority')}] {record.get('title')} - Read: {record.get('is_read')}")
            else:
                print(f"\n{table.upper()} table: Error retrieving count")
